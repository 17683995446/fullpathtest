#!/usr/bin/env python3
"""
FullPathTest V21.0 - 超详细日志插桩 + 超长时超大规模极端测试
"""
...
class V21UltraLoggingTestSystem:
    ...
    # ========================================================================
    # Phase 2: 超长时连续测试（1000次）
    # ========================================================================
    
    def phase2_ultra_long_duration_test(self):
        """Phase 2: 超长时连续测试（1000次）"""
        self.stage = 2
        self.log_section("Phase 2: 超长时连续测试（1000次调用）")
        ...
        for i in range(1, self.max_iterations + 1):
            try:
                ...
                if i % 100 == 0:
                    ...
                    if len(durations) > 0:
                        avg_duration = sum(durations[-100:]) / min(100, len(durations))
                        self.log(f"最近100次平均耗时: {avg_duration:.3f}秒", "INFO")
    ...
    # ========================================================================
    # Phase 3: 超大规模并发测试（50并发）
    # ========================================================================
    
    def phase3_ultra_concurrent_test(self):
        """Phase 3: 超大规模并发测试（50并发）"""
        self.stage = 3
        self.log_section("Phase 3: 超大规模并发测试（50并发）")
        ...
        def run_concurrent_task(task_id: int):
            try:
                ...
                return is_success, task_duration, None
            except Exception as e:
        ...
        with ThreadPoolExecutor(max_workers=self.max_concurrent) as executor:
            ...
            for future in as_completed(futures):
                completed += 1
                is_success, duration, error = future.result()
                if is_success:
                    success_count += 1
                else:
                    failed_count += 1
    ...
    def phase5_mixed_pressure_test(self):
        ...
        for i in range(100):
            try:
                ...
                if i % 25 == 24:
                    self.log(f"混合测试进度: {i+1}/100, 成功: {success_count}", "INFO")
    ...
    def run_all_tests(self):
        ...
        self.log_section("FullPathTest V21.0 - 超详细日志+超长时+超大规模+混乱测试")
    ...
    def print_summary(self):
        ...
        if self.discovered_issues:
            ...
            for idx, issue in enumerate(self.discovered_issues[:20], 1):  # 只显示前20个
